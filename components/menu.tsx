...
export default function Menu() {
  ...
  const [adminData, setAdminData] = useState<{ institute_name?: string; institute_code?: string } | null>(null);

  // Synchronize session state across all page navigations so clicking Home or other pages keeps user logged in
  useEffect(() => {
    const checkSession = () => {
      ...
      if (sessionStr) {
        ...
        try {
          ...
          setAdminData({ institute_code: 'Admin', institute_name: 'Manavta Admin' });
        }
      } else {
        setIsLoggedIn(false);
        setAdminData(null);
  ...
  return (
    <header className="w-full sticky top-0 z-50 bg-black shadow-md font-sans">
        ...
        <div className="hidden lg:flex items-center gap-6">
          ...
          <div className="relative">
            <button 
              ...
              className="flex items-center gap-1 text-md font-bold text-white hover:text-orange-200 transition focus:outline-none py-1.5 cursor-pointer"
            >
              About Us
          ...
          {/* 6. Enroll Now */}
          <Link href="/enroll-now" className="text-md font-bold text-amber-300 hover:text-amber-100 transition">
            Enroll Now ⚡
          </Link>
              ...
              >
                ...
                <div className="w-10 h-10 rounded-full bg-sky-100 text-sky-700 font-black flex items-center justify-center border-2 border-sky-400 group-hover:border-sky-600 transition shadow-sm overflow-hidden">
                  <span className="text-sm font-bold">
                    {adminData?.institute_code ? adminData.institute_code.slice(0, 2).toUpperCase() : 'MV'}
                  </span>
                </div>
                ...
                <>
                  ...
                  <div className="absolute right-0 mt-3.5 w-60 bg-white rounded-xl shadow-xl border border-slate-100 py-2.5 z-20 animate-in fade-in slide-in-from-top-2 duration-150">
                    <div className="px-4 py-2 border-b border-slate-100 mb-1.5 bg-slate-50/80 rounded-t-xl">
                      ...
                      <p className="text-xs font-bold text-slate-800 truncate">{adminData?.institute_name || 'Manavta Admin'}</p>
                      <p className="text-[10px] font-mono font-semibold text-sky-700">Code: {adminData?.institute_code || 'ADMIN'}</p>
                    </div>
                    <Link 
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden w-full bg-white border-t border-slate-100 py-5 px-6 animate-in fade-in slide-in-from-top duration-200 text-slate-800">
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col border-l-2 border-slate-100 pl-3 py-1 gap-2">
              ...
              <Link href="/verify" className="text-xs font-semibold text-slate-700" onClick={() => setIsMobileMenuOpen(false)}>• Verify Registration</Link>
              ...
              {!isLoggedIn ? (
                ...
                <div className="flex items-center justify-between bg-slate-50 p-3 rounded-xl">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <p className="text-xs font-bold text-slate-800 leading-none">{adminData?.institute_name || 'Manavta Admin'}</p>
                      <p className="text-[9px] text-emerald-600 font-semibold mt-0.5">Logged In</p>
                    </div>
                  </div>
                  <button 
                    onClick={handleLogout}
                    className="text-xs font-bold text-rose-600 bg-white py-1 px-3 rounded-lg border border-rose-100 shadow-sm cursor-pointer"
                  >
                    Logout
                  </button>
                </div>
